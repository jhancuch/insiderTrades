...
from insiderTrades.utils.formFilterDerivativeTransactions import formFilterDerivativeTransactions
from insiderTrades.utils.transactionFilterDerivative import transactionFilterDerivative
...
def derivativeTransactionsPullAndScrape(quarter, year, form, name, email, transactionType = None, footnoteKeywords = None, issuerKeywords = None, issuerTradingSymbol = None, rptOwnerKeywords = None):
    """
    ...
    for i in pull:
        ...
        filingCleaned = re.sub(r'<nonDerivativeTable.*?</nonDerivativeTable>|<derivativeHolding.*?</derivativeHolding>|<value>|</value>',
        ...
        keyCount = formFilterDerivativeTransactions(filing = filingCleaned, footnoteKeywords = footnoteKeywords, issuerKeywords = issuerKeywords, issuerTradingSymbol = issuerTradingSymbol, rptOwnerKeywords = rptOwnerKeywords, transactionType = transactionType)
        ...
        if keyCount > 0 or (footnoteKeywords is None and issuerKeywords is None and issuerTradingSymbol is None and rptOwnerKeywords is None and transactionType is None):
            ...
            if filingCount == 1 and peopleCount == 1:
                ...
                keyTransactionCount = transactionFilterDerivative(filing = filingCleaned, footnoteKeywords = footnoteKeywords, issuerKeywords = issuerKeywords, issuerTradingSymbol = issuerTradingSymbol, rptOwnerKeywords = rptOwnerKeywords, transactionType = transactionType)
                ...
                if keyTransactionCount > 0 or (footnoteKeywords is None and issuerKeywords is None and issuerTradingSymbol is None and rptOwnerKeywords is None and transactionType is None):
                    ...
                    dat = transactionScrape(filing = filingCleaned, counter = counter, dat = dat, n = 0, k = 27, columnNumber = 0, columnNames = columnNames)
                    dat = footnoteScrape(filing = filingCleaned, counter = counter, dat = dat, n = 28, k = 58, footnoteNumber = 1, columnNumber = 28, transactionType = "derivativeTransaction", columnNames = columnNames)
                    dat.iloc[counter, 58] = i
                ...
                while transactionCount >= 1:
                    ...
                    filingCleaned = re.sub("<derivativeTransaction>.*?</derivativeTransaction>", "", filingCleaned, count = 1, flags = re.MULTILINE | re.DOTALL)
                    transactionCount = len(re.findall("<securityTitle>", filingCleaned, flags = re.MULTILINE | re.DOTALL))
                ...
                while peopleCountTwo >= 1:
                    ...
                    if keyTransactionCount > 0 or (footnoteKeywords is None and issuerKeywords is None and issuerTradingSymbol is None and rptOwnerKeywords is None and transactionType is None):
                        ...
                        for columnName in columnNames[28:58]:
                        ...
                        dat = footnoteScrape(filing = filingCleaned, counter = counter, dat = dat, n = 28, k = 58, footnoteNumber = 1, columnNumber = 28, transactionType = "derivativeTransaction", columnNames = columnNames)
                        dat.iloc[counter, 58] = i
                        dat.iloc[counter, 60] = "The transaction values in this observation is an aggregate amount that is shared by the other observations that share the same URL"
                ...
                while transactionCount >= 1:
                    ...
                    if keyTransactionCount > 0 or (footnoteKeywords is None and issuerKeywords is None and issuerTradingSymbol is None and rptOwnerKeywords is None and transactionType is None):
                        ...
                        dat = transactionScrape(filing = filingCleaned, counter = counter, dat = dat, n = 12, k = 27, columnNumber = 12, columnNames = columnNames)

                        dat = footnoteScrape(filing = filingCleaned, counter = counter, dat = dat, n = 28, k = 58, footnoteNumber = 1, columnNumber = 28, transactionType = "derivativeTransaction", columnNames = columnNames)
                        dat.iloc[counter, 59] = i
                        dat.iloc[counter, 60] = "This transaction may not be a valid to key word conditions based upon the structure of many reporting owners. This transaction must be checked by hand."
