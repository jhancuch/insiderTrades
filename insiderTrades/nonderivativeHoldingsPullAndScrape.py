...
from insiderTrades.secUrlDownload import secUrlDownload
from insiderTrades.utils.formFilterNonderivativeHoldings import formFilterNonderivativeHoldings
from insiderTrades.utils.transactionFilterNonderivativeHoldings import transactionFilterNonderivativeHoldings
...
def nonderivativeHoldingsPullAndScrape(quarter, year, form, name, email, footnoteKeywords = None, issuerKeywords = None, issuerTradingSymbol = None, rptOwnerKeywords = None):
    """
    ...
    dat = pd.DataFrame(index=np.arange(len(pull)*20), columns = columnNames)
    ...
    for i in pull:
        ...
        filingCleaned = re.sub(r'<derivativeTable.*?</derivativeTable>|<nonDerivativeTransaction.*?</nonDerivativeTransaction>|<value>|</value>',
        ...
        keyCount = formFilterNonderivativeHoldings(filing = filingCleaned, footnoteKeywords = footnoteKeywords, issuerKeywords = issuerKeywords, issuerTradingSymbol = issuerTradingSymbol, rptOwnerKeywords = rptOwnerKeywords)
        ...
        if keyCount > 0 or (footnoteKeywords is None and issuerKeywords is None and issuerTradingSymbol is None and rptOwnerKeywords is None):
            ...
            if filingCount == 1 and peopleCount == 1:
                ...
                keyTransactionCount = transactionFilterNonderivativeHoldings(filing = filingCleaned, footnoteKeywords = footnoteKeywords, issuerKeywords = issuerKeywords, issuerTradingSymbol = issuerTradingSymbol, rptOwnerKeywords = rptOwnerKeywords)
                ...
                if keyTransactionCount > 0 or (footnoteKeywords is None and issuerKeywords is None and issuerTradingSymbol is None and rptOwnerKeywords is None):
                    ...
                    dat = transactionScrape(filing = filingCleaned, counter = counter, dat = dat, n = 0, k = 15, columnNumber = 0, columnNames = columnNames)
                    dat = footnoteScrape(filing = filingCleaned, counter = counter, dat = dat, n = 16, k = 46, footnoteNumber = 1, columnNumber = 16, transactionType = "nonDerivativeHolding", columnNames = columnNames)
                    dat.iloc[counter, 46] = i
                ...
                while transactionCount >= 1:
                    ...
                    filingCleaned = re.sub("<nonDerivativeHolding>.*?</nonDerivativeHolding>", "", filingCleaned, count = 1, flags = re.MULTILINE | re.DOTALL)
                    transactionCount = len(re.findall("<securityTitle>", filingCleaned, flags = re.MULTILINE | re.DOTALL))
                ...
                while peopleCountTwo >= 1:
                    ...
                    if keyTransactionCount > 0 or (footnoteKeywords is None and issuerKeywords is None and issuerTradingSymbol is None and rptOwnerKeywords is None):
                        ...
                        for columnName in columnNames[16:46]:
                        ...
                        dat = footnoteScrape(filing = filingCleaned, counter = counter, dat = dat, n = 16, k = 46, footnoteNumber = 1, columnNumber = 21, transactionType = "nonDerivativeHolding", columnNames = columnNames)
                        dat.iloc[counter, 46] = i
                        dat.iloc[counter, 48] = "The holding values in this observation is an aggregate amount that is shared by the other observations that share the same URL"
                ...
                while transactionCount >= 1:
                    ...
                    if keyTransactionCount > 0 or (footnoteKeywords is None and issuerKeywords is None and issuerTradingSymbol is None and rptOwnerKeywords is None):
                        ...
                        dat = transactionScrape(filing = filingCleaned, counter = counter, dat = dat, n = 11, k = 15, columnNumber = 11, columnNames = columnNames)

                        dat = footnoteScrape(filing = filingCleaned, counter = counter, dat = dat, n = 16, k = 46, footnoteNumber = 1, columnNumber = 16, transactionType = "nonDerivativeHolding", columnNames = columnNames)
                        dat.iloc[counter, 47] = i
                        dat.iloc[counter, 48] = "This transaction may not be a valid to key word conditions based upon the structure of many reporting owners. This holding must be checked by hand."
