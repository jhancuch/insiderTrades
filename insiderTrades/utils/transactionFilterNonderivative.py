...
def transactionFilterNonderivative(filing, footnoteKeywords = None, issuerKeywords = None, issuerTradingSymbol = None, rptOwnerKeywords = None, transactionType = None):
    """
    This function checks if the specific transaction being examined contains any of the keywords
    Parameters:
    ...
    # This section checks if the transaction on the form being examined contain any of the footnote keywords. For each key word identified, a value of 1 is added to the score.
    # Any transaction with a score above 1 is then examined by the rest of the script.
    ...
    if footnoteKeywords is not None:
        ...
        filingTransactionSection = re.search(r'<nonDerivativeTransaction>.*?</nonDerivativeTransaction>', filing, flags = re.MULTILINE | re.DOTALL)[0]
    ...
    if transactionType is not None:
        ...
        transactionTypeSection = re.search('<nonDerivativeTransaction>.*?</nonDerivativeTransaction>', filing, flags = re.MULTILINE | re.DOTALL)[0]
